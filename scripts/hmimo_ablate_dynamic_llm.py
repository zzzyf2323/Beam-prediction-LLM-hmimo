"""Controlled dynamic ablation harness for LLM-assisted HMIMO temporal priors."""
...
def _run_sbl(fwd, adj, y, shape, groups, noise_var: float, cfg: Dict[str, object], gamma_init: float):
    return solve_group_sbl(
        ...
        gamma_init=max(float(gamma_init), 1e-8),
    )
...
def main() -> int:
    ...
    llm_prior = HMIMOLLMTemporalPrior(
        LLMTemporalPriorConfig(
            enabled=True,
            history_len=int(cfg["llm_prior"]["history_len_full"]),
            fusion_dim=int(cfg["llm_prior"]["fusion_dim"]),
            seed=int(cfg["llm_prior"]["seed"]),
        )
    )

    variants = [
        {"name": "fpws+group_sbl_no_temporal", "type": "no"},
        {"name": "fpws+group_sbl_nonllm_temporal", "type": "nonllm"},
        {"name": "fpws+group_sbl_llm_full", "type": "llm", "ablation": {}},
        {
            "name": "fpws+group_sbl_llm_no_metadata",
            "type": "llm",
            "ablation": {"use_metadata": False},
        },
        {
            "name": "fpws+group_sbl_llm_no_prev_gamma",
            "type": "llm",
            "ablation": {"use_gamma": False},
        },
        {
            "name": "fpws+group_sbl_llm_no_prev_energy",
            "type": "llm",
            "ablation": {"use_energy": False},
        },
        {
            "name": "fpws+group_sbl_llm_short_history",
            "type": "llm",
            "ablation": {"history_len": int(cfg["llm_prior"]["history_len_short"])},
        },
    ]
    ...
    for trial in range(n_mc):
        ...
        state = {
            v["name"]: {"ha": None, "gamma": None, "history": []}
            for v in variants
        }
        ...
        for snr_db in snr_list:
            for frame_idx, (ha_true, h_true) in enumerate(zip(ha_seq, h_seq)):
                ...
                nrng = np.random.default_rng(seed + 10000 * trial + int(100 * snr_db) + frame_idx)
                ...
                for variant in variants:
                    ...
                    if variant["type"] == "no":
                        gamma_init = 1.0
                    elif variant["type"] == "nonllm":
                        warm = temporal_warm_start_from_prev(
                            prev_ha_hat=vstate["ha"],
                            groups=groups,
                            prev_gamma=vstate["gamma"],
                            alpha=float(cfg["temporal_prior"]["alpha"]),
                        )
                        gamma_init = float(np.mean(np.asarray(warm["gamma_group_init"], dtype=float)))
                    else:
                        warm = temporal_warm_start_from_prev(
                            prev_ha_hat=vstate["ha"],
                            groups=groups,
                        ...
                        prior = llm_prior.infer_prior(
                            history=vstate["history"],
                            metadata=meta,
                            fallback_gamma=fallback,
                            ablation=variant.get("ablation", {}),
                        )
                        gamma_init = float(np.mean(np.asarray(prior["gamma_init"], dtype=float)))

                    out = _run_sbl(fwd, adj, y, shape, groups, noise_var, cfg["group_sbl"], gamma_init)
                    ha_hat = out["ha_hat"]
                    ...
                    gamma_arr = np.asarray(out.get("gamma_group"), dtype=float)
                    energy = np.asarray(
                        [float(np.mean(np.abs(np.asarray(ha_hat).reshape(-1)[np.asarray(g)] ** 2))) for g in groups],
                        dtype=float,
                    )
                    active_prob = np.clip(gamma_arr / (np.max(gamma_arr) + 1e-12), 0.0, 1.0)

                    vstate["ha"] = ha_hat
                    vstate["gamma"] = gamma_arr
                    vstate["history"].append(
                        {
                            "gamma": gamma_arr,
                            "energy": energy,
    ...
    print("Dynamic LLM-prior ablation summary")
    for rec in summary:
        print(
            f"{rec['scheme']:<34} | SNR={rec['snr_db']:>5.1f} | "
